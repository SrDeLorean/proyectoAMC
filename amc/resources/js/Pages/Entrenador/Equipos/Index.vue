...
function editarEquipo() {
  router.get(`/entrenador/equipos/${props.equipo.id}/edit`)
}
const esDueno = computed(() => {
  return page.props.auth?.user?.id === props.equipo?.propietario?.id
})
...
<template>
  <EntrenadorLayout>
    ...
    <div class="p-4 sm:p-6 lg:p-8 space-y-6">
      <div
        v-if="equipo"
        class="rounded p-6 shadow text-white flex flex-col md:flex-row md:items-start gap-4 relative"
      ...
      >
        ...
        <!-- Botón Editar usando Lucide -->
        <button
          v-if="esDueno"
          @click="editarEquipo"
          class="absolute bottom-4 left-4 p-2 rounded hover:bg-blue-700 transition"
          type="button"
