...
const props = defineProps({
  temporadas: Object,  // paginadas
  success: String,
  trashed: Boolean     // true si estamos en /trashed
})

// Columnas base para temporadas (id y nombre)
const baseColumns = [
  { label: 'ID', key: 'id' },
...
function onTableAction({ actionName, row }) {
  if (actionName === 'edit') {
    router.get(`/temporadas/${row.id}/edit`)
  }
  ...
  if (actionName === 'delete') {
    Swal.fire({
      ...
      if (result.isConfirmed) {
        router.delete(`/temporadas/${row.id}`)
      }
    })
  }
  ...
  if (actionName === 'restore') {
    Swal.fire({
      ...
      if (result.isConfirmed) {
        router.post(`/temporadas/${row.id}/restore`)
      }
    })
  }
}
...
<template>
  <AdminLayout>
    <template #title>
      {{ trashed ? 'Temporadas Eliminadas' : 'Temporadas' }}
    </template>
    ...
    <div class="p-6 space-y-6">
      ...
      <div class="flex justify-between items-center">
        ...
        <div class="flex gap-3">
          ...
          <template v-else>
            <a
              href="/temporadas/create"
              class="bg-red-600 hover:bg-red-700 text-white px-4 py-2 rounded transition"
            >
              + Crear Temporada
            </a>
            <a
              href="/temporadas/trashed"
              class="bg-gray-700 hover:bg-gray-800 text-white px-4 py-2 rounded transition"
            >
              🗑️ Ver Eliminadas
            </a>
