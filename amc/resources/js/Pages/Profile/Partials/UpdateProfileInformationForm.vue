...
<template>
<section class="bg-gray-900 rounded-xl shadow-lg px-10 py-10 flex flex-col md:flex-row gap-10 border border-gray-800 w-full max-w-5xl mx-auto">
  <!-- Imagen de perfil -->
  <div class="flex flex-col items-center md:items-start md:w-1/4 text-center md:text-left">
    <img
      ...
      @error="onImageError"
    />
    <label for="foto" class="text-sm text-gray-300 mb-2 font-medium cursor-pointer">Cambiar imagen</label>
    <input
      id="foto"
      type="file"
      accept="image/*"
      @change="onFileChange"
      class="file:bg-red-600 file:hover:bg-red-700 file:border-0 file:text-white file:rounded file:px-4 file:py-1 text-sm text-gray-300 bg-gray-800 border border-gray-700 rounded-md w-full cursor-pointer"
    />
  ...
  <div class="md:w-3/4 space-y-6">
    ...
    <div v-if="mustVerifyEmail && user.email_verified_at === null" class="bg-gray-800 p-4 rounded-lg border border-gray-700">
    ...
    <div class="flex items-center gap-4">
      <PrimaryButton :disabled="form.processing" class="bg-red-600 hover:bg-red-700 text-white text-base px-6 py-2" @click="submit">
        Guardar cambios
      </PrimaryButton>
